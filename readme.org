...
+ /Basic.ino/: provides basic functions.
  ...
  - =void startRGBCycle( uint8_t s_colors[], uint32_t s_durations[] )=: sets the colors and durations for the color cycle. =s_colors= should be an array with r,g,b values from 0 to 255, =s_durations= an array with the time (in ms) to reach the defined color (length of the array should thus be length of =*s_durations= / 3).

  - =void startFadeWhiteLight( uint8_t intens[], uint32_t durations[] )=: same as above, just for the white light. Also, since only a single light intensity is defined instead of r, g and b values, the length of =*intens= and =*durations= have to match.
